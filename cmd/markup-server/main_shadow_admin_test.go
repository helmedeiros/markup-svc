...
func TestE2E_ShadowAdminLifecycleLoadAndClear(t *testing.T) {
	...
	handler, _, err := wireTracedHandler(loader, body, nil, guardrailsWire{}, metricsWiring{}, observabilityWire{}, nil, true, 1.0, 0)
	if err != nil {
...
func TestE2E_ShadowAdminRoutesAbsentWhenDisabled(t *testing.T) {
	...
	handler, _, err := wireTracedHandler(loader, body, nil, guardrailsWire{}, metricsWiring{}, observabilityWire{}, nil, false, 1.0, 0)
	if err != nil {
		t.Fatalf("wireTracedHandler: %v", err)
	}
	srv := httptest.NewServer(handler)
