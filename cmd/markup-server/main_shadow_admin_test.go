...
func TestE2E_ShadowAdminLifecycleLoadAndClear(t *testing.T) {
	...
	handler, _, err := wireTracedHandler(loader, body, nil, guardrailsWire{}, metricsWiring{}, nil, nil, true, 1.0, 0, "", nil)
	if err != nil {
...
func TestE2E_ShadowAdminRoutesAbsentWhenDisabled(t *testing.T) {
	...
	handler, _, err := wireTracedHandler(loader, body, nil, guardrailsWire{}, metricsWiring{}, nil, nil, false, 1.0, 0, "", nil)
	if err != nil {
		t.Fatalf("wireTracedHandler: %v", err)
	}
	srv := httptest.NewServer(handler)
