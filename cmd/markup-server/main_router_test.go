...
func TestE2ERouterAsymmetryOverHTTP(t *testing.T) {
	...
	srv := httptest.NewServer(wireRouterHandler(r, nil, holders, guardrailsWire{}, metricsWiring{}))
	t.Cleanup(srv.Close)
...
func TestE2ERouterReloadWithoutHoldersReturns404(t *testing.T) {
	...
	srv := httptest.NewServer(wireRouterHandler(r, nil, nil, guardrailsWire{}, metricsWiring{}))
	t.Cleanup(srv.Close)
...
func TestE2ERouterModeGuardrailsAdminMounts(t *testing.T) {
	...
	srv := httptest.NewServer(wireRouterHandler(r, nil, holders, gw, metricsWiring{}))
	t.Cleanup(srv.Close)
