...
func TestE2ERouterAsymmetryOverHTTP(t *testing.T) {
	...
	srv := httptest.NewServer(wireRouterHandler(r, nil, holders, guardrailsWire{}, metricsWiring{}, nil, "", nil))
	t.Cleanup(srv.Close)
...
func TestE2ERouterReloadWithoutHoldersReturns404(t *testing.T) {
	...
	srv := httptest.NewServer(wireRouterHandler(r, nil, nil, guardrailsWire{}, metricsWiring{}, nil, "", nil))
	t.Cleanup(srv.Close)
...
func TestE2ERouterModeGuardrailsAdminMounts(t *testing.T) {
	...
	srv := httptest.NewServer(wireRouterHandler(r, nil, holders, gw, metricsWiring{}, nil, "", nil))
	t.Cleanup(srv.Close)
