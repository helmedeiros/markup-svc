...
- Two consumers of the same `markup.decision.v1` payload now exist when a sink is wired: the structured-log emission AND the typed sink. The middleware computes the `ts` string once and hands it to both builders; it reuses the same `inputFields` map for `request_context` so the second build does not re-allocate the nested map. The remaining per-Decide cost when a sink is wired is one `decisionsink.Event` struct plus one interface dispatch; the exact byte cost is an analytic estimate (~136 B stack-allocated struct + string-backing storage) until the scientific harness lands a measurement.
- Default deployment cost on the access-log path is zero from the sink: `WithAccessLog` captures a `sinkEnabled` boolean at construction by type-asserting against `NoopSink`. When the cmd wiring passes nil or `NoopSink{}`, the per-request hot path skips the Event build, skips the interface call, and pays no sink-related allocation. The pre-existing `statusRecorder` heap allocation in `WithAccessLog` is unaffected by this ADR.
- Per-Decide cost of the sink-enqueue path when a substrate is wired remains unmeasured at the time of this ADR. A `BenchmarkSinkPublish` bar pre-registration is listed in **Not closed** as a follow-on harness commit; the default-deployment 0-sink-allocation claim above is the matching bar's pre-registered floor.

### Not closed (deferred to follow-on ADRs)

- **`BenchmarkSinkPublish` + `BenchmarkSinkPublishNoop` pre-registration.** Follow-on harness commit lands the bars under `scientific/v0.1.X/` (next-available release directory) with the 0-allocation floor for the NoopSink path and a per-call ceiling for the wired path. Same ADR-0012 parking pattern as ADR-0035.
